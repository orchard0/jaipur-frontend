...
const EndTurn = ({
	...
	const validHandSelection = () => {
		...
		// check that every good selected is of the same type
			...
			!selectedHandCards.every((card) => card.card_type === card_type)
		) {
	...
	const validSwapSelection = () => {
		...
		const outcomeSwap =
			handCardLength() -
			selectedHandCards.filter((card) => card.card_type !== 'Camel')
				.length +
			selectedTableCards.filter((card) => card.card_type !== 'Camel')
				.length;
	...
	return (
		<button
			...
			onClick={handleClick}>
			{turnEnded ? `${actionLookup[action]}` : "Opponent's turn..."}
		</button>
	);
};
